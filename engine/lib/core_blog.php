...
function getPosts( $bloguri, $post_directory, $start, $end){
	...
	if (! $start <= 0) echo "<a href='/?url=".$bloguri.'/home/start/'.($start - getConfigOption('posts_per_page') )."/end/".($end - getConfigOption('posts_per_page') )."'>Newer Posts</a>";
	if (! $start <= 0 and count($posts) != $i ) echo ' / ';
	if ( count($posts) != $i ) echo "<a href='/?url=".$bloguri.'/home/start/'.($start + getConfigOption('posts_per_page') )."/end/".($end + getConfigOption('posts_per_page') )."'>  Older Posts</a>  ";
